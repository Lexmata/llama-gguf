//! Test NeoX-style RoPE vs standard RoPE.
...
/// Standard RoPE: consecutive pairs (x[0], x[1]), (x[2], x[3]), etc.
fn apply_rope_standard(data: &mut [f32], pos: usize, head_dim: usize, freq_base: f32) {
...
/// NeoX-style RoPE: first half paired with second half (x[0], x[half]), (x[1], x[half+1]), etc.
...
fn run_full_forward(
    ...
    bias_before_rope: bool,  // Also test bias order
) -> Vec<f32> {
    let hidden_size = 896;
    let num_heads = 14;
    let head_dim = 64;
    ...
    let apply_rope: fn(&mut [f32], usize, usize, f32) = match rope_style {
        RopeStyle::Standard => apply_rope_standard,
        RopeStyle::NeoX => apply_rope_neox,
    };
...
fn main() {
    ...
    println!("Testing RoPE styles and bias ordering with tokens: {:?}", tokens);
    println!("(Looking for token 17 '2' to have high rank)");
    println!();
    
    // Test all combinations
    let combinations = vec![
        (RopeStyle::Standard, false, "Standard RoPE + Bias AFTER RoPE"),
        (RopeStyle::Standard, true, "Standard RoPE + Bias BEFORE RoPE"),
        (RopeStyle::NeoX, false, "NeoX RoPE + Bias AFTER RoPE"),
        (RopeStyle::NeoX, true, "NeoX RoPE + Bias BEFORE RoPE"),
    ];
    
    for (rope_style, bias_before, name) in combinations {
        let logits = run_full_forward(
            &tokens, &gguf, &backend, &emb, &output_norm_w, &output_w,
            rope_style, bias_before
        );
        ...
        println!("=== {} ===", name);
        println!("  Token 17 ('2') rank: {}", rank_17);
        println!("  Token 17 logit: {:.4}", logits[17]);
        println!("  Top 5 tokens: {:?}", indexed.iter().take(5).map(|(i, l)| (*i, format!("{:.2}", l))).collect::<Vec<_>>());
        println!();
    }
}
