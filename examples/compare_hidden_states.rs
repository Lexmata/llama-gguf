...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let token_id: u32 = args.get(1).and_then(|s| s.parse().ok()).unwrap_or(28); // Default to '=' token
    ...
    eprintln!(
        "Processing token {} through {} layers...",
        token_id, num_layers
    );
    ...
    // Output JSON
    let output: Value = states.into_iter().map(|(k, v)| (k, json!(v))).collect();
