//! Test different orderings of RoPE and bias application.
//!
...
/// Option A: bias_before_rope = true  -> Q = RoPE(Wq @ x + bias)
/// Option B: bias_before_rope = false -> Q = RoPE(Wq @ x) + bias
fn run_inference(gguf: &GgufFile, backend: &CpuBackend, bias_before_rope: bool) -> (f32, usize) {
    let hidden_size = 896;
    let num_heads = 14;
    let head_dim = 64;
    let num_kv_heads = 2;
...
fn main() {
    ...
    println!();
    println!("Option B: Q = RoPE(Wq @ x) + bias [bias NOT rotated]");
    println!("  Token 17 logit: {:.4}, rank: {}", logit_b, rank_b);
    println!();
    println!("Reference: llama.cpp produces rank 1, logit ~18");
    println!();
    if rank_b < rank_a {
        println!("Option B is better! Perhaps llama.cpp adds bias AFTER RoPE.");
    } else if rank_a < rank_b {
        println!("Option A is better. Our original approach is correct.");
    } else {
        println!("Both options produce similar results.");
    }
}
