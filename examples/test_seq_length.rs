//! Test how the error scales with sequence length.
...
fn run_inference(gguf: &GgufFile, backend: &CpuBackend, tokens: &[u32]) -> (f32, usize) {
    let hidden_size = 896;
    let num_heads = 14;
    let head_dim = 64;
    let num_kv_heads = 2;
...
fn main() {
    ...
    println!("=== Sequence Length Test ===");
    println!();
    println!("Tokens                  | Seq Len | Token 17 Logit | Rank");
    println!("------------------------+---------+----------------+-------");

    // Test different prefixes before "=" to see if error accumulates
    let tests: Vec<(Vec<u32>, &str)> = vec![
        (vec![28], "="),                           // Just "="
        (vec![16, 28], "1="),                      // "1="
        (vec![16, 10, 28], "1+="),                 // "1+="
        (vec![16, 10, 16, 28], "1+1="),            // "1+1="
        (vec![16, 10, 16, 10, 16, 28], "1+1+1="),  // "1+1+1="
    ];

    for (tokens, desc) in tests {
        let (logit, rank) = run_inference(&gguf, &backend, &tokens);
        println!("{:23} | {:>7} | {:>14.4} | {:>5}", 
            format!("{:?} ({})", tokens, desc), 
            tokens.len(), 
            logit, 
    ...
    println!();
    println!("Note: If rank stays consistent, the error is per-layer.");
    println!("      If rank degrades with length, error accumulates.");
}
