//! Test running only a subset of layers to find where divergence starts.
...
fn run_inference(gguf: &GgufFile, backend: &CpuBackend, num_layers_to_run: usize) -> (f32, usize) {
    let hidden_size = 896;
    let num_heads = 14;
    let head_dim = 64;
    let num_kv_heads = 2;
    ...
    for pos in 0..tokens.len() {
        for layer_idx in 0..num_layers_to_run {
...
fn main() {
    ...
    println!("=== Partial Layer Test ===");
    println!("Testing prediction of '2' (token 17) after '1+1='");
    println!();
    println!("Layers | Token 17 Logit | Token 17 Rank");
    println!("-------+----------------+--------------");

    for num_layers in [1, 2, 4, 8, 12, 16, 20, 24] {
        let (logit, rank) = run_inference(&gguf, &backend, num_layers);
        println!("{:>6} | {:>14.4} | {:>13}", num_layers, logit, rank);
    }

    println!();
    println!("Reference: llama.cpp produces Token 17 Rank = 1");
    println!("If rank is 1 early but degrades with more layers,");
    println!("the error accumulates. If rank is bad from layer 1,");
    println!("the initial computation is wrong.");
}
