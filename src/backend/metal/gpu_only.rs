...
impl MetalGpuInference {
    pub fn from_model(model: LlamaModel, max_seq_len: usize) -> BackendResult<Self> {
        ...
        let ffn_down = ctx.create_output_buffer(config.intermediate_size)?;
        ...
        let moe_temp = ctx.create_output_buffer(scratch_size)?;
    ...
    pub fn forward(&mut self, token_id: u32) -> BackendResult<Vec<f32>> {
        ...
        let mut logits = self.ctx.read_buffer(&self.logits, self.config.vocab_size);
    ...
    fn process_layer(&mut self, layer_idx: usize) -> BackendResult<()> {
        ...
        if matches!(&self.layers[layer_idx].attn_layer, AttentionLayer::Mamba(_)) {
            return Err(BackendError::OperationFailed(
                "Mamba SSM not yet supported on Metal GPU".into(),
            ));
        }
    ...
    fn attention_forward(&mut self, layer_idx: usize) -> BackendResult<()> {
        ...
        let q_bias = format!("blk.{}.attn_q.bias", layer_idx);
        let k_bias = format!("blk.{}.attn_k.bias", layer_idx);
        let v_bias = format!("blk.{}.attn_v.bias", layer_idx);
        self.linear_gpu(
            &format!("blk.{}.attn_q.weight", layer_idx),
            self.weights.contains(&q_bias).then_some(q_bias.as_str()),
            BufRef::HiddenNorm,
            BufRef::AttnQ,
        )?;
        self.linear_gpu(
            &format!("blk.{}.attn_k.weight", layer_idx),
            self.weights.contains(&k_bias).then_some(k_bias.as_str()),
            BufRef::HiddenNorm,
            BufRef::AttnK,
        )?;
        self.linear_gpu(
            &format!("blk.{}.attn_v.weight", layer_idx),
            self.weights.contains(&v_bias).then_some(v_bias.as_str()),
            BufRef::HiddenNorm,
            BufRef::AttnV,
        )?;
    ...
    /// LayerNorm via Metal compute shader (single workgroup, threadgroup reduction).
    fn layer_norm_gpu(
        &self,
        weight_name: &str,
        bias_name: &str,
        ...
        let param_bytes = unsafe {
            std::slice::from_raw_parts(
                ...
                std::mem::size_of::<LayerNormParams>(),
            )
        };

        let tg_count = MTLSize::new(1, 1, 1);
        let tg_size = MTLSize::new(256, 1, 1);
        self.ctx.dispatch_threadgroups(
            "layer_norm",
            &[x_buf, &w.buffer, &b.buffer, out_buf],
            Some((param_bytes, 4)),
            tg_count,
            tg_size,
        )?;
    ...
    fn dispatch_gelu(&self, buf: &Buffer, n: usize) -> BackendResult<()> {
        let mut push = Vec::with_capacity(4);
        push.extend_from_slice(&(n as u32).to_le_bytes());
        let (grid, tg) = MetalContext::sizes_1d(n);
        self.ctx.dispatch("gelu", &[buf, &self.moe_temp], Some((&push, 2)), grid, tg)?;
        self.ctx.copy_buffer(&self.moe_temp, buf, bytes_of_f32(n))
    }
    ...
    fn dispatch_rope(
        ...
        push.extend_from_slice(&(rope_dims as u32).to_le_bytes());
...
impl crate::backend::GpuInference for MetalGpuInference {
    fn forward(&mut self, token_id: u32) -> crate::backend::BackendResult<Vec<f32>> {
        MetalGpuInference::forward(self, token_id)
    }
    fn prefill_token(&mut self, token_id: u32) -> crate::backend::BackendResult<()> {
        MetalGpuInference::prefill_token(self, token_id)
    }
    fn reset(&mut self) {
        MetalGpuInference::reset(self)
    }
    fn position(&self) -> usize {
        MetalGpuInference::position(self)
    }
}
